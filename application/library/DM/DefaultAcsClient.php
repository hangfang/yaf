 ...
 * under the License.
 */
class DM_DefaultAcsClient implements DM_IAcsClient 
{	
	...
	{	
		...
		{
			throw new DM_Exception_ClientException("No active profile found.", "SDK.InvalidProfile");
		}
		if(null == $iSigner)
		{
		...
		$domain = DM_Regions_EndpointProvider::findProductDomain($request->getRegionId(), $request->getProduct());
		if(null == $domain)
		{
			throw new DM_Exception_ClientException("Can not find endpoint to access.", "SDK.InvalidRegionId");
		}
		$requestUrl = $request->composeUrl($iSigner, $credential, $domain);
		if(count($request->getDomainParameter())>0){
			$httpResponse = DM_Http_HttpHelper::curl($requestUrl, $request->getMethod(), $request->getDomainParameter(), $request->getHeaders());
		} else {
			$httpResponse = DM_Http_HttpHelper::curl($requestUrl, $request->getMethod(), null, $request->getHeaders());
		}
		...
		while (500 <= $httpResponse->getStatus() && $autoRetry && $retryTimes < $maxRetryNumber) {
			...
			if(count($request->getDomainParameter())>0){
			    $httpResponse = DM_Http_HttpHelper::curl($requestUrl, $request->getDomainParameter(), $request->getHeaders());
			} else {
				$httpResponse = DM_Http_HttpHelper::curl($requestUrl, $request->getMethod(), null, $request->getHeaders());
			}
			$retryTimes ++;
	...
	private function buildApiException($respObject, $httpStatus)
	{
		if(500 <= $httpStatus)
		{
			throw new DM_Exception_ServerException($respObject->Message, $respObject->Code);
		}
		else
		{
			throw new DM_Exception_ClientException($respObject->Message, $respObject->Code);
		}
	}
