 ...
 * under the License.
 */
class DM_Profile_DefaultProfile implements DM_Profile_IClientProfile
{
	...
	{
		$credential =new DM_Auth_Credential($accessKeyId, $accessSecret);
		self::$profile = new DM_Profile_DefaultProfile($regionId, $credential);
		return self::$profile;
	}
	...
	public function getSigner()
	{
		...
		{
			self::$isigner = new DM_Auth_ShaHmac1Signer(); 
		}
		return self::$isigner;
	}
	...
	public static function getEndpoints()
	{
		...
		{
			self::$endpoints = DM_Regions_EndpointProvider::getEndpoints();
		}
	...
	private static function addEndpoint_($endpointName,$regionId, $product, $domain)
	{
		...
		$productsDomains = array(new DM_Regions_ProductDomain($product, $domain));
		$endpoint = new DM_Regions_Endpoint($endpointName, $regionIds, $productDomains);
	...
	private static function updateEndpoint($regionId, $product, $domain, $endpoint)
	{
		...
		{
		 	array_push($productDomains, new DM_Regions_ProductDomain($product, $domain));	
		}
