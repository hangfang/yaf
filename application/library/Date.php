...
class Date {
    ...
    public function timeDiff( $time ,$precision=false) {
        ...
        //static $chunks = array(array(31536000,'年'),array(2592000,'个月'),array(604800,'周'),array(86400,'天'),array(3600 ,'时'),array(60,'分'),array(1,'秒'));
        static $chunks = array(array(3600 ,'小时'),array(60,'分钟'),array(1,'秒'));
        $count =0;
        $since = '';
        for($i=0;$i<count($chunks);$i++) {
            if($diff > 86400){
                return $since = date('m月d日', strtotime($time));
            }
            if($diff>=$chunks[$i][0]) {
