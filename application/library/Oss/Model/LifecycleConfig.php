 ...
 */
class Oss_Model_LifecycleConfig implements Oss_Model_XmlConfig
{
    /**
    ...
    public function parseFromXml($strXml)
    {
        ...
        foreach ($xml->Rule as $rule) {
            ...
            foreach ($rule as $key => $value) {
                ...
                $actions[] = new Oss_Model_LifecycleAction($action, $timeSpec, $timeValue);
            }
            $this->rules[] = new Oss_Model_LifecycleRule($id, $prefix, $status, $actions);
        }
        return;
    }
    ...
    public function addRule($lifecycleRule)
    {
        if (!isset($lifecycleRule)) {
            throw new Oss_Core_Exception("lifecycleRule is null");
        }
