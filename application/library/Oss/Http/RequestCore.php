    ...
    public function streaming_read_callback($curl_handle, $file_handle, $length)
    {
        ...
        if ($this->read_stream_read == 0 && isset($this->seek_position) && $this->seek_position !== ftell($this->read_stream)) {
            if (fseek($this->read_stream, $this->seek_position) !== 0) {
                throw new Oss_Http_RequestCoreException('The stream does not support seeking and is either not at the requested position or the position is unknown.');
            }
        }
...
//		curl_setopt($curl_handle, CURLOPT_CLOSEPOLICY, CURLCLOSEPOLICY_LEAST_RECENTLY_USED);
            ...
            case self::HTTP_PUT:
                ...
                if (isset($this->read_stream)) {
                    if (!isset($this->read_stream_size) || $this->read_stream_size < 0) {
                        throw new Oss_Http_RequestCoreException('The stream size for the streaming upload cannot be determined.');
                    }
    ...
    public function process_response($curl_handle = null, $response = null)
    {
        ...
        if (is_resource($this->curl_handle)) {
            ...
            if ($curl_handle && $response) {
                ...
                return new Oss_Http_ResponseCore($this->response_headers, $this->response_body, $this->response_code);
            }
        }
    ...
    public function send_request($parse = false)
    {
        ...
        if ($this->response === false) {
            throw new Oss_Http_RequestCoreException('cURL resource: ' . (string)$curl_handle . '; cURL error: ' . curl_error($curl_handle) . ' (' . curl_errno($curl_handle) . ')');
        }
    ...
    public function send_multi_request($handles, $opt = null)
    {
        ...
        do {
            ...
            while ($done = curl_multi_info_read($multi_handle)) {
                ...
                if ($done['result'] > 0) {
                    throw new Oss_Http_RequestCoreException('cURL resource: ' . (string)$done['handle'] . '; cURL error: ' . curl_error($done['handle']) . ' (' . $done['result'] . ')');
